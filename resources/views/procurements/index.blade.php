...
@section('title', 'Procurements - Nigeria Hydrological Services Agency')
...
<style>
    ...
    .Procurements-hero .hero-content {
        position: relative;
        z-index: 2;
    }
    ...
    .procurement-card:hover .procurement-icon::before {
    ...
    .procurement-list-item:hover {
        transform: translateX(15px);
        box-shadow: var(--shadow-medium);
        border-left-color: #0056b3;
    }

    .procurement-list-item:hover::before {
        transform: scaleY(1);
    }
    ...
    .procurement-list-meta {
        color: #6c757d;
        font-size: 0.95rem;
        line-height: 1.5;
    }

    .procurement-list-actions {
        display: flex;
        align-items: center;
        gap: 0.75rem;
    }
...
@section('content')
    ...
    <!-- Hero Section -->
    <section class="Procurements-hero py-5">
        <div class="container">
            <div class="row align-items-center">
                <div class="col-lg-8">
                    <div class="hero-content">
                        <h1 class="display-4 fw-bold mb-4">Procurements</h1>
                        <p class="lead mb-4">Access our comprehensive collection of hydrological reports, research papers, bulletins, and technical Procurements</p>
                            ...
                            <div class="col-md-4">
                                <div class="stats-card card text-center p-4 stats-primary">
                                    <div class="card-body">
                                        ...
                                        <h2 class="text-white mb-2">{{ number_format($procurements->total()) }}</h2>
                                        <p class="mb-0 text-white-50">Total Procurements</p>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="stats-card card text-center p-4 stats-success">
                                    <div class="card-body">
                                        ...
                                        <p class="mb-0 text-white-50">procurement Types</p>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-4">
                ...
                <div class="col-lg-4">
                    <div class="search-box position-relative">
                        <div class="search-container">
                            ...
                            <input type="text" class="form-control form-control-lg search-input" id="search-filter" placeholder="Search Procurements...">
            ...
            <div class="row">
                ...
                <div class="col-lg-3 mb-4 mb-lg-0">
                    <div class="filter-card card mb-4">
                        <div class="card-header">
                            <h5 class="mb-0"><i class="fas fa-filter me-2"></i>Filter Procurements</h5>
                        </div>
                        <div class="card-body">
                            <form id="filter-form">
                                <div class="mb-4">
                                ...
                                <div class="mb-4">
                                    <label for="type-filter" class="form-label fw-bold">
                                        <i class="fas fa-tags me-2"></i>procurement Type
                                    </label>
                    ...
                    <div class="help-card card">
                        <div class="card-body text-center">
                            ...
                            <p class="mb-3">Can't find what you're looking for? Request specific data or Procurements.</p>
                ...
                <div class="col-lg-9">
                    ...
                    <div class="alert alert-info" style="display: none;">
                        <h6>Debug Info:</h6>
                        <p><strong>Total Procurements:</strong> {{ $procurements->total() }}</p>
                        <p><strong>Current Page Count:</strong> {{ $procurements->count() }}</p>
                        <p><strong>Procurements by Type Count:</strong> {{ count($procurementsByType) }}</p>
                        <p><strong>Types:</strong> {{ implode(', ', $types) }}</p>
                        <p><strong>Years:</strong> {{ implode(', ', $years) }}</p>
                        <p><strong>Procurements by Type:</strong></p>
                        <pre>{{ json_encode($procurementsByType, JSON_PRETTY_PRINT) }}</pre>
                    </div>

                    <!-- Featured Procurements -->
                    <div class="mb-5">
                        <h2 class="section-title">Featured Procurements</h2>
                        <div class="row g-4">
                            @php
                                $featuredProcurements = $procurements->filter(function($pub) {
                                    return $pub->is_featured;
                                });
                            @endphp

                            @forelse($featuredProcurements as $index => $procurement)
                                <div class="col-md-6 col-lg-4 featured-item-{{ $index }}">
                                    <div class="procurement-card card position-relative">
                                        ...
                                        <div class="card-body text-center p-4">
                                            ...
                                            <h5 class="card-title mb-3">{{ $procurement->title }}</h5>
                                            <div class="d-flex justify-content-center align-items-center gap-2 mb-3">
                                                <span class="procurement-type-badge">{{ $procurement->type }}</span>
                                                <span class="badge bg-light text-dark border">
                                                    <i class="fas fa-calendar me-1"></i>{{ $procurement->year }}
                                                </span>
                                            </div>
                            ...
                            @empty
                                <div class="col-12">
                                    <div class="empty-state">
                                        ...
                                        <h4>No Featured Procurements</h4>
                                        <p>Check back later for featured Procurements from NIHSA.</p>
                                    </div>
                                </div>
                            @endforelse
                        </div>
                    ...
                    <!-- Procurements by Type -->
                    @forelse($procurementsByType as $type => $typeProcurements)
                        <div class="mb-5 procurement-section" id="section-{{ $type }}">
                            <h2 class="section-title">{{ $type }}</h2>
                            <div class="row g-3">
                                @foreach($typeProcurements as $procurement)
                                    <div class="col-12">
                                        <div class="procurement-list-item card">
                                            <div class="procurement-list-content">
                    ...
                    @empty
                        <div class="col-12">
                            <div class="empty-state">
                                <i class="fas fa-file-alt"></i>
                                <h4>No Procurements Available</h4>
                                <p>We don't have any Procurements to display at the moment. Please check back later or contact us for specific data requests.</p>
...
@section('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            console.log('Procurements page loaded');

            const applyFiltersBtn = document.getElementById('apply-filters');
            const yearFilter = document.getElementById('year-filter');
            const typeFilter = document.getElementById('type-filter');
            const searchFilter = document.getElementById('search-filter');
            const Procurementsections = document.querySelectorAll('.procurement-section');
            const procurementCards = document.querySelectorAll('.procurement-card');
            const procurementListItems = document.querySelectorAll('.procurement-list-item');

            console.log('Found elements:', {
                Procurementsections: Procurementsections.length,
                procurementCards: procurementCards.length,
                procurementListItems: procurementListItems.length
            });
            ...
            function applyFilters(year, type, search) {
                ...
                Procurementsections.forEach(section => {
                ...
                // Filter procurement cards by year and search term
                procurementCards.forEach(card => {
                    const cardYear = card.querySelector('.procurement-meta .text-muted').textContent.trim();
                ...
                // Filter procurement list items by year and search term
                procurementListItems.forEach(item => {
                    const itemYear = item.querySelector('.procurement-type-badge').textContent;
            ...
            function checkEmptyResults() {
                const visibleCards = Array.from(procurementCards).filter(card =>
                    card.style.display !== 'none' && card.style.opacity !== '0'
                );

                const visibleItems = Array.from(procurementListItems).filter(item =>
                    item.style.display !== 'none' && item.style.opacity !== '0'
                );

                const visibleSections = Array.from(Procurementsections).filter(section =>
                    section.style.display !== 'none'
                );
                ...
                if (visibleCards.length === 0 && visibleItems.length === 0 && visibleSections.length === 0) {
                    ...
                    console.log('No Procurements match the current filters');
                } else {
                    console.log('Found Procurements:', visibleCards.length, 'cards,', visibleItems.length, 'items,', visibleSections.length, 'sections');
                }
            }
            ...
            // Observe all procurement cards and list items
            procurementCards.forEach(card => observer.observe(card));
            procurementListItems.forEach(item => observer.observe(item));
            ...
            style.textContent = `
                ...
                .procurement-list-item:hover .procurement-list-info h5 {
                    color: var(--primary-color);
                    transition: color 0.3s ease;
                }
